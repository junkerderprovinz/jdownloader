...
agent cannot reach them. Setting the colours in FlatLaf's own dark properties is the
only place that covers everything, race-free.
...
Progress bars: FlatProgressBarUI paints the fill from the bar's foreground, which is the
accent-derived @accentSliderColor. We make the fill LIGHT (via the master @accentBaseColor,
the only variable that cascades — @accentColor/@accentSliderColor are recomputed at runtime
and don't stick) and invert the % text so it is DARK where it overlaps the light fill and
LIGHT over the dark track (ProgressBar.selectionForeground / selectionBackground). That is
how the Material Darker theme makes the bar readable, with no custom UI class.
...
MARKER = "#jdp-carbon-dark v6"             # bump when OVERRIDES change -> forces re-patch

OVERRIDES = """

#jdp-carbon-dark v6 - complete #161616 monochrome dark; LIGHT progress fill + dark % over it.
# The progress fill is @accentSliderColor = if(@accentColor, @accentColor, @accentBase2Color),
# and @accentBase2Color = lighten(... @accentBaseColor ...). FlatLaf recomputes @accentColor at
# runtime (systemColor(accent) = null on Linux), so only the MASTER @accentBaseColor cascades:
# @accentBaseColor -> @accentBase2Color -> @accentSliderColor -> ProgressBar fill. A LIGHT
# @accentBaseColor therefore gives a light bar; the % text is inverted dark over the fill via
# ProgressBar.selectionForeground and stays light over the dark track via selectionBackground.
@background = #161616
@foreground = #f4f4f4
@componentBackground = #1e1e1e
@disabledForeground = #6f6f6f
@accentBaseColor = #c6c6c6
@selectionBackground = #525252
@selectionForeground = #f4f4f4
ProgressBar.background = #262626
ProgressBar.selectionForeground = #161616
...
def main():
    ...
    try:
        ...
        with open(DEP, "w") as f:
            json.dump(dep, f)
        log("patched FlatDarkLaf -> #161616 dark, light progress fill (v6); dep.json sha256 updated")
    except Exception as e:
